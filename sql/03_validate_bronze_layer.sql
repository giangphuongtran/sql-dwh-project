...
BEGIN
    ...
    INSERT INTO monitoring.data_validation_logs (table_name, record_key, column_name, error_reason, detected_at)
    SELECT 'crm_cust_info', CAST(cst_id AS TEXT), 'cst_gndr', 'Unexpected gender', current_timestamp
    FROM bronze.crm_cust_info
    WHERE UPPER(TRIM(cst_gndr)) NOT IN ('F', 'M');

    -- crm_cust_info: invalid marital status
    INSERT INTO monitoring.data_validation_logs (table_name, record_key, column_name, error_reason, detected_at)
    SELECT 'crm_cust_info', CAST(cst_id AS TEXT), 'cst_marital_status', 'Unexpected status', current_timestamp
    FROM bronze.crm_cust_info
